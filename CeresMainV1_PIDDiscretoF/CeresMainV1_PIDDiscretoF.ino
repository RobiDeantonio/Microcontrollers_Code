...
// Version:       21.0
// Authors:       Adrien Legrand (V.20) & Guillermo Sánchez (V.21)
// Organization:  Universidad Nacional de Colombia, Universidad Militar Nueva Grenada
// Proyecto:      Proyecto de Grado - 2018-II & Proy 2019 UMNG
// Goal:          Run open-loop and closeloop algorithm to control the CERES Agrobot with ROS
// Date:          16-01-2020
// ############################################################################################
//
//  SUMMARY:
...
#define minAngVel -0.4 // Maximum Robot Angular Velocity [rad/s] antes 0.25

#define maxVel 2.0    // Maximum Robot Linear Velocity [m/s] antes 1.0
#define maxAngVel 0.4 // Maximum Robot Angular Velocity [rad/s]
...
// ############################################################################################
ros::NodeHandle nh;
//ceres::CeresArduinoLogging log_msg; OLD topic for debug, deprecated for Matlab compatibility
geometry_msgs::Vector3 log_msg2;
//ros::Publisher log_pub("/ceres/arduLog", &log_msg); OLD topic for debug, deprecated for Matlab compatibility
...
void setup() {
  ...
  tcontrol.setInterval(ControlTime);
  tcontrol.setCallback(applycontrol);
  tcontrol.start();
...
// ############################################################################################
void loop() {
  //if(digitalRead(ES_1_PIN) || digitalRead(ES_1_PIN)) { Futura si se desea colocar paradas de emergencias
  //  mode = 0;
  //}
  // Log Changed Mode
  if (lastmode != mode) {
    ...
    case 0: // RC/Manual Mode
      if (millis() < lastRCReceivedTime + RCTimeout) { // Verify that a RC data has been received //
        ...
        if (closeLoop) {
          ...
          applySpeed(u, w);
          //log_msg.Ul=u; Old implementation of ceres/Ardulog topic
...
// ############################################################################################
// 1. applycontrol: CallBack called each ControlTime miliseconds Only if t.control.update is called un the loop.
// ############################################################################################
...
// ############################################################################################
// 2. ReceiveRefPose: CallBack called each time a reference is received as a Twist message.
// ############################################################################################
void receiveRefPose( const geometry_msgs::Twist& cmd_vel) {
...
// ############################################################################################
// 3. ReceivePose: Callback called each time a Twist2D Message is received from the IMU.
// ############################################################################################
void receivePose(const geometry_msgs::Vector3& pose_msg) {
  Usens = pose_msg.x;
  Wsens = pose_msg.z;
...
// ############################################################################################
// 4. ReceiveRC: CallBack called each time RC values are received as a CeresRC message.
// ############################################################################################
...
// ############################################################################################
// 5. PIDu: Calculate the PID output value for U.
// ############################################################################################
float PIDu(float Uin) {
  
  float Uerror = Uin - Usens;
  unsigned long newTime=millis();
  float Upu = Pu * (Uerror - lastUerr);
  float Uiu = (Iu * Uerror * ControlTime / 1000.0);
  float Udu = Du * (Uerror - lastUerr) * 1000.0 /ControlTime;
  float Uout = Upu + Uiu + Udu + lastUout;
  if(Uout > 0.8) {
    Uout = 0.8;
    USatFlag=true;
  }
...
// ############################################################################################
// 6. PIDw: Calculate the PID output value for W.
// ############################################################################################
float PIDw(float Win) {
  
  float error = Win - Wsens;
  unsigned long newTime=millis();
  float Wpu = Pw * (error-lastWerr);
  float Wiu = (Iw * error * ControlTime / 1000.0);
  float Wdu = Dw * (error - lastWerr) * 1000.0 /ControlTime;
  float Wout = Wpu + Wiu + Wdu + lastWout;
  if(Wout > 0.3) {
    Uout = 0.3;
    WSatFlag=true;
  }
...
// ############################################################################################
// 7. applySpeed: 1st-order transfer function that convert the speeds to drivers voltages
//                and apply them
// ############################################################################################
void applySpeed(float u, float w) {
  ...
  //register the control signal in /ceres/ControlSignal topic
  log_msg2.z = w;
  log_msg2.y = 0.0;
  log_msg2.x = u;
...
// ############################################################################################
// 8. DAC_set: Realiza la comunicación con los DAC, enviando el voltaje calculado (input)
// al modulo seleccionado DAC_sel
// ############################################################################################
